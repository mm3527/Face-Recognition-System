...
def contact():
    mess._show(title='Contact us', message="Please contact us on : 'mmurlimuz007@gmail.com' ")
...
def TrackImages():
    ...
    while True:
        ...
        cv2.imshow('Taking Info', im)
        if (cv2.waitKey(1) == ord('q')):
            break
    ts = time.time()
    date = datetime.datetime.fromtimestamp(ts).strftime('%d-%m-%Y')
    exists = os.path.isfile("Info\Info_" + date + ".csv")
    if exists:
        with open("Info\Info_" + date + ".csv", 'a+') as csvFile1:
            writer = csv.writer(csvFile1)
        ...
        with open("Info\Info_" + date + ".csv", 'a+') as csvFile1:
            ...
            writer.writerow(attendance)
        csvFile1.close()
    with open("Info\Info_" + date + ".csv", 'r') as csvFile1:
        reader1 = csv.reader(csvFile1)
...
message3 = tk.Label(window, text="Face Recognition System" ,fg="white",bg="#2d420a" ,width=55 ,height=1,font=('comic', 29, ' bold '))
...
trackImg = tk.Button(frame1, text="Click Here", command=TrackImages  ,fg="black"  ,bg="#3ffc00"  ,width=35  ,height=1, activebackground = "white" ,font=('comic', 15, ' bold '))
